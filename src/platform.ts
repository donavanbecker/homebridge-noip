  ...
  private async createContactSensor(device) {
    ...
    if (existingAccessory) {
      ...
      if (!this.config.disablePlugin) {
        ...
        existingAccessory.context.serialNumber = (await si.networkInterfaces().then(data => (`${data[1].ip4}`)));
      ...
      accessory.context.serialNumber = (await si.networkInterfaces().then(data => (`${data[1].ip4}`)));
